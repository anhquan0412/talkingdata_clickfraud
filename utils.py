...
def merge_mean_to_val(val_df,glob_mean,prev_day):
	...
	for filename in mean_filenames:
		...
		val_df[new_fea] = downcast_1col(val_df[new_fea])
		del mean_df
		gc.collect()
...
def groupby_agg(spec,X_train,X_val=None):
	...
	X_train[new_feature] = downcast_1col(X_train[new_feature])        
	if not X_val is None:
		...
		X_val[new_feature] = downcast_1col(X_val[new_feature])       
...
def next_prev_click(df,cols,next_fea,prev_fea):
	...
	df[prev_fea]=(df.clicktime_sec - df.groupby(cols).clicktime_sec.shift(1)).astype(np.float32)
	#df[prev_fea].fillna(-1,inplace=True)

	df.drop('clicktime_sec',axis=1,inplace=True)
    
	df[next_fea] = downcast_1col(df[next_fea])
	df[prev_fea] = downcast_1col(df[prev_fea])
	gc.collect()
	return df
...
def get_val_by_name(cols_to_drop,name):
	...
	gc.collect()
	return val_df,y_val
def get_train(cols_to_drop,filename,sz=3000000):
	train_df=get_sample_timeseries(filename,sz)
